            ...
            if (detail && detail.displacement !== null && detail.displacement !== undefined) {
                ...
                if (brand === 'GEA Grasso' && detail.rpm_range && Array.isArray(detail.rpm_range) && detail.rpm_range.length === 2) {
                    ...
                    modelDisplacementInfoLp.innerHTML = `
                        <span class="font-bold">理论流量:</span> <span id="model_displacement_value_m6_lp">${displacement.toFixed(0)}</span> m³/h
                    ...
                    modelDisplacementInfoLp.innerHTML = `
                        <span class="font-bold">理论排量:</span> <span id="model_displacement_value_m6_lp">${displacement.toFixed(0)}</span> m³/h
                    `;
                }
            ...
            if (detail && detail.displacement !== null && detail.displacement !== undefined) {
                ...
                if (brand === 'GEA Grasso' && detail.rpm_range && Array.isArray(detail.rpm_range) && detail.rpm_range.length === 2) {
                    ...
                    modelDisplacementInfoHp.innerHTML = `
                        <span class="font-bold">理论流量:</span> <span id="model_displacement_value_m6_hp">${displacement.toFixed(0)}</span> m³/h
                        <span class="ml-2 text-xs text-gray-600">(最大转速 ${maxRpm} RPM)</span>
                        <br>
                        <span class="text-xs text-gray-600">转速范围: ${minRpm}-${maxRpm} RPM</span>
                    `;
                    ...
                    modelDisplacementInfoHp.innerHTML = `
                        <span class="font-bold">理论排量:</span> <span id="model_displacement_value_m6_hp">${displacement.toFixed(0)}</span> m³/h
                    `;
                }
